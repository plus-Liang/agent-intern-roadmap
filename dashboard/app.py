...
# 云端（Streamlit Cloud / Python 3.14）进程的 stdout/stderr 是 ascii 或 latin-1 时，
# 任何带中文的 print 都会抛 UnicodeEncodeError（'ascii'/'latin-1' codec can't
# encode characters in position ...），并把真正的错误盖掉。入口处统一改成 UTF-8，
# 覆盖全进程所有 print / 日志。
# 注意：这里不再设 PYTHONIOENCODING / PYTHONUTF8——解释器早已启动，`os.environ`
# 改这两个变量不会重新生效，真正起作用的是下面的 reconfigure。
